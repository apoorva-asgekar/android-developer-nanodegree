...
public class ChildActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent intentThatCreatedChildActivity = getIntent();

        // Create an if statement to check if this Intent has the extra we passed from MainActivity
        if (intentThatCreatedChildActivity.hasExtra(Intent.EXTRA_TEXT)) {
            String stringEnteredByUser =
                    intentThatCreatedChildActivity.getStringExtra(Intent.EXTRA_TEXT);
            mDisplayText.setText(stringEnteredByUser);
        }
