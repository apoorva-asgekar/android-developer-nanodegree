...
public class MainActivity extends AppCompatActivity {
    ...
    public void onClickOpenAddressButton(View v) {
        ...
        String myAddress =  "geo:0,0?q=1600+Amphitheatre+Parkway%2C+CA";
        String someAddress = "2010 Latham Street, CA";

        // Use Uri.Builder with the appropriate scheme and query to form the Uri for the address
        Uri.Builder builder = new Uri.Builder();
        builder.scheme("geo")
                .appendPath("0,0")
                .appendQueryParameter("q", someAddress);
        Uri addressUri = builder.build();

        Log.v("MainActivity.class", addressUri.toString());

        //  Replace the Toast with a call to showMap, passing in the Uri from the previous step
        //showMap(Uri.parse(myAddress));
        showMap(addressUri);
    }
