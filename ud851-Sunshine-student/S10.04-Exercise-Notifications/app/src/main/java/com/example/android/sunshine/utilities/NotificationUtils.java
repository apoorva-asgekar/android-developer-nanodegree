...
//  TODOCOMPLETED (1) Create a constant int value to identify the notification
    private static final int NOTIFICATION_ID  = 3456;

    private static final int PENDING_INTENT_REQUEST_CODE = 232;
...
//          TODOCOMPLETED (2) Use NotificationCompat.Builder to begin building the notification
...
//          TODOCOMPLETED (3) Create an Intent with the proper URI to start the DetailActivity
            Intent detailActivityIntent = new Intent (context, DetailActivity.class);
            detailActivityIntent.setData(todaysWeatherUri);

//          TODOCOMPLETED (4) Use TaskStackBuilder to create the proper PendingIntent
            ...
            PendingIntent pendingIntent =
                    taskStackBuilder.getPendingIntent(PENDING_INTENT_REQUEST_CODE, PendingIntent.FLAG_UPDATE_CURRENT);

            builder.setContentIntent(pendingIntent);

//          TODOCOMPLETED (5) Set the content Intent of the NotificationBuilder

//          TODOCOMPLETED (6) Get a reference to the NotificationManager
            NotificationManager notificationManager =
                    (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);

//          TODOCOMPLETED (7) Notify the user with the ID WEATHER_NOTIFICATION_ID
            notificationManager.notify(NOTIFICATION_ID, builder.build());

//          TODOCOMPLETED (8) Save the time at which the notification occurred using SunshinePreferences
